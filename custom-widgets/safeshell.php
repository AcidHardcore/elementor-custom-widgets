...
class Safeshell extends Widget_Base
{
    ...
    public function __construct($data = [], $args = null)
    {
        ...
        $css_version = $theme_version . '.' . filemtime(get_template_directory() . '/css/widget-safeshell.css');
        wp_register_style('widget-safeshell', get_template_directory_uri() . '/css/widget-safeshell.css', array('main-styles'), $css_version);

    }

    public function get_style_depends()
    {
        return ['widget-safeshell'];
    }
    ...
    {
        return __('Safeshell', 'elementor-custom-widgets');
    }
    ...
    protected function register_controls()
    {
        ...
        $this->start_controls_section('section_intro', [
            'label' => __('Safeshell', 'elementor-custom-widgets'),
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .safeshell__text > *' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .safeshell__text > *',
            ]
        );
    ...
    protected function render()
    {
      ...
      <div id="<?= esc_attr($section_id) ?>" class="section safeshell <?= esc_attr($section_class) ?>">
          ...
          <div class="grid-row grid-row--nog grid-row--aic">
            <div data-wow-delay="0.25s" class="grid-col grid-col--sm-12 wow fadeInUp">
                <?php if (isset($settings['logo']) && !empty($settings['logo']['id'])) : ?>
                    <?= wp_get_attachment_image($settings['logo']['id'], 'full', false, array('class' => 'safeshell__logo')) ?>
                <?php endif; ?>

                <?php if (isset($settings['text'])) : ?>
                  <div class="mt-100 safeshell__text"><?= wp_kses_post($settings['text']) ?></div>
                <?php endif; ?>
            ...
            <div class="grid-col grid-col--auto grid-col--sm-order-0 wow fadeInUp">
              <div class="safeshell__figure">
                  <?php if (isset($settings['image']) && !empty($settings['image']['id'])) : ?>
                      <?= wp_get_attachment_image($settings['image']['id'], 'full', false, array('class' => 'safeshell__image')) ?>
                  <?php endif; ?>

                  <?php if (isset($settings['number']) && !empty($settings['number']['id'])) : ?>
                    <div class="safeshell__num"><?= file_get_contents(wp_get_original_image_path($settings['number']['id'])); ?></div>
                  <?php endif; ?>
              </div>
            </div>
          </div>
